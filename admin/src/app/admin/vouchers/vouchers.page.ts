...
export class VouchersPage implements OnInit {
  ...
  async initializeSubscriptions() {
    ...
    this.events.subscribe('voucher:publishAllProductsForAdminProducts', async (products) => {
      this.allProductsData = products;
      this.showNoProducts = false;
      this.showSearchLoader = false;
      if (!this.categories) {
        this.categories = await this.voucherService.getAllCategoriesForSideMenu()
      }
      if (!this.brands) {
        this.brands = await this.brandService.getAllBrandsForSideMenu()
      }
      if (!this.allSubcategories) {
        this.allSubcategories = await this.voucherService.getAllSubcategoriesForSideMenu()
      }
      this.exportProducts()
    });
  ...
  async loadMoreCategoryProducts() {
    await this.presentLoading();
    const products: any = await this.voucherService.loadMoreCategoryProducts(this.selectedSubcatId);
  ...
  async checkForProdLimit() {
    const allProds: any = await this.voucherService.getAllProductsForVendor();
  ...
  async selectListItem(i) {
    ...
    if (this.dataType == 'categories') {
      ...
      let data: any = await this.voucherService.getCategoryProducts(this.dataList[i].id)
      if (data) {
    ...
    if (this.dataType == 'brands') {
      let data: any = await this.voucherService.getBrandProducts(this.dataList[i].id)
      if (data) {
        this.productsData = data
      }
  ...
  async selectSubCat(i, j) {
    ...
    this.noMoreCategoryProducts = false;
    let data: any = await this.voucherService.getCategoryProducts(this.dataList[i].sublist[j].id)
    if (data) {
      this.productsData = data
    }
  }
  ...
  async selectSubOfSubCat(i: number, j: number, k: number) {
    ...
    this.noMoreCategoryProducts = false;
    let data: any = await this.voucherService.getCategoryProducts(this.dataList[i].sublist[j].subOfSubCatList[k].id);
    if (data) {
      this.productsData = data;
    }
  }
