...
import { UserCartDetailsModalPage } from '../user-cart-details-modal/user-cart-details-modal.page';

@Component({
  selector: 'app-users-cart',
  templateUrl: './users-cart.page.html',
  styleUrls: ['./users-cart.page.scss'],
})
export class UsersCartPage implements OnInit {
  ...
  getDateTimeFormat(date){
    ...
    let formatedDate:any = this.sharedService.convertInvalidDateObjectToTimestamp(date);
  ...
  async openModal(uid: string, udata:any) {
    ...
    const closed = await modal.onWillDismiss();
