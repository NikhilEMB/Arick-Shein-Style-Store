...
export class BookingPage implements OnInit {
  ...
  async initializeSubscriptions() {
    ...
    this.events.subscribe('booking:publishAllProductsForAdminProducts', async (products) => {
      this.allProductsData = products;
      this.showNoProducts = false;
      this.showSearchLoader = false;
      if (!this.categories) {
        this.categories = await this.bookingService.getAllCategoriesForSideMenu()
      }
      if (!this.brands) {
        this.brands = await this.brandService.getAllBrandsForSideMenu()
      }
      if (!this.allSubcategories) {
        this.allSubcategories = await this.bookingService.getAllSubcategoriesForSideMenu()
      }
      this.exportProducts()
    });
  ...
  async loadMoreCategoryProducts() {
    await this.presentLoading();
    const products: any = await this.bookingService.loadMoreCategoryProducts(this.selectedSubcatId);
  ...
  async checkForProdLimit() {
    const allProds: any = await this.bookingService.getAllProductsForVendor();
  ...
  async selectListItem(i) {
    ...
    if (this.dataType == 'categories') {
      ...
      let data: any = await this.bookingService.getCategoryProducts(this.dataList[i].id)
      if (data) {
    ...
    if (this.dataType == 'brands') {
      let data: any = await this.bookingService.getBrandProducts(this.dataList[i].id)
      if (data) {
        this.productsData = data
      }
  ...
  async selectSubCat(i, j) {
    ...
    this.noMoreCategoryProducts = false;
    let data: any = await this.bookingService.getCategoryProducts(this.dataList[i].sublist[j].id)
    if (data) {
      this.productsData = data
    }
  }
  ...
  async selectSubOfSubCat(i: number, j: number, k: number) {
    ...
    this.noMoreCategoryProducts = false;
    let data: any = await this.bookingService.getCategoryProducts(this.dataList[i].sublist[j].subOfSubCatList[k].id);
    if (data) {
      this.productsData = data;
    }
  }
