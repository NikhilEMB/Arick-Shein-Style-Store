...
let OrderSummaryPage = class OrderSummaryPage {
    constructor(events, router, alertController, loadingController, storage, actionSheetController, camera, imagePicker, navCtrl) {
        this.events = events;
        this.router = router;
    ...
    getDeliveryDates(days) {
        ...
        for (let index = 0; index < this.maxDaysOfDelivery; index++) {
            let dayToCheck = moment__WEBPACK_IMPORTED_MODULE_7__().add(index + 1, 'days');
    ...
    appendSameDateToDeliveryDates(lastTime) {
        let now = moment__WEBPACK_IMPORTED_MODULE_7__().format('HH:mm');
        //console.log('now', now);
        let lastDeliveryTime = moment__WEBPACK_IMPORTED_MODULE_7__(lastTime, ["hh:mm A"]).format('HH:mm');
    ...
    disableTime(time) {
        if (this.selectedDate) {
            if (this.selectedDate.getDate() !== new Date().getDate()) {
                ...
                let now = moment__WEBPACK_IMPORTED_MODULE_7__().format('HH:mm');
                let startTime = moment__WEBPACK_IMPORTED_MODULE_7__(time.start, ["hh:mm A"]).format('HH:mm');
                if (now > startTime) {
                    return true;
                }
            }
...
//# sourceMappingURL=order-summary-order-summary-module-es2015.js.map